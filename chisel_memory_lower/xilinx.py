...
def generate(config: Config, tb: bool, vivado_path):
    ...
    if tb:
        with open(f'{config.name}_xilinx.prj', 'w') as file:
            ...
            print(
                f'sv work {vivado_path}/data/ip/xpm/xpm_memory/hdl/xpm_memory.sv', file=file)

        with open(f'{config.name}_xilinx.sh', 'w') as file:
            print(f'#!/bin/bash', file=file)
            print(
                f'export PATH={vivado_path}/bin:$PATH', file=file)
            print(
